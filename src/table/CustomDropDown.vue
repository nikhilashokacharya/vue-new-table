<template>
  <div class="custom-dropdown-outer">
    ...
    height: 17px;">
        <select 
        @change="changedVal($event,data)" 
        @focus="focus"
        @blur="blur"
        >
          <option v-for="data1 in data.option" :key="data1" :value="data1">{{data1}}</option>
        </select>
        <input type="text" name="format" :value="data.value" />
      </div>
    </div>
  </div>
</template>
...
export default class CustomDropDown extends Vue {
  ...
  emitMe: any;
  mounted(){
    this.data.value=this.data.option[0];
  }
  changedVal(e:any, data: any){
    this.data.value=e.target.value;
    this.emitMe=data;
    this.$emit("change",e, this.emitMe);

  }
  focus(e: any, data: any) {
    this.$emit("focus",e, data);
  }
  blur(e: any, data: any) {
